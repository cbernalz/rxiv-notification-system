...
ARXIV_API = "http://export.arxiv.org/api/query?search_query={query}&start=0&max_results=5&sortBy=submittedDate&sortOrder=descending"

def fetch_arxiv_papers(keywords, authors_filter=None):
    keyword_query = "+AND+".join([f"all:{kw.replace(' ', '+')}" for kw in keywords]) if keywords else ""
